...
<!-- ----------------------------------------------------------------- -->
	...
	<div id="payment_Context">
		...
		<table  class="table table-hover">
			...
			<tr class="table_rows_num">
				<td hidden>${goodsno }</td>
				<td hidden>${price }</td>
				<td>${imgsrc }</td>
				<td id="table_td_goodsname">${goodsname }<br/>
					<p id="table_context">
					COLOR : ${color }<br/>
					GENDER : ${gender }<br/>
					SIZE : ${size }</p>
				</td>
				<td class="table_context_middle">${amount }</td>
				<td class="table_context_middle" id="table_rows_num_sum">${price*amount }</td>
				<!-- 추가 -->
				<td hidden>${goodsno }</td>
				<td hidden>${SessionScope.id }</td>
	...
	<div id ="menu_bar">
		<div id="menu_bar_font">주문금액 확인및결제</div>
	...
	<div id="menu_bar2">
		<div id="menu_bar2_headline">주문동의|주문내용을 확인해주세요.</div>
